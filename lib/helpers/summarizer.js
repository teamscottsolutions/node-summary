...
export function getSortedSentences (paragraph, cached, n, filter) {
  // const sentences = splitContentToSentences(paragraph)
  // if (!sentences) return ''
  // if (sentences.length < 2) return ''

  let scores = Object.entries(cached)
  if (filter) {
    scores = scores.filter(entry => filter(entry[1].text))
  }
  scores = scores.map((entry, order) => ({
    sentence: entry[1].text,
    score: entry[1].score,
    order
  }))
    .filter(s => s)
...
export function getSentencesRanks (content, cached, filter) {
  ...
  return nRange.reduce((cached, i) => {
    ...
    return Object.assign(cached, { [sentence]: { score, text: sentences[i] } })
  }, {})
}
