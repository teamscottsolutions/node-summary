...
export function sortedSentences (content, n, cached, filter) {
  if (typeof n === 'object') {
    cached = n
    filter = cached
    n = 0
  ...
  const dict = getSentencesRanks(content, cached, filter)
  const sentences = getSortedSentences(content, dict, n, filter)
